...
def add_features(dataset: pd.DataFrame, cols_to_condense: list = None, condensed_col_name: str = None,
                 ...
                 windowsize_rolling: int = None, windowsize_rolling_seas: int = None,
    ...
    if windowsize_rolling is None:
        windowsize_rolling = seasonal_periods
    if windowsize_rolling_seas is None:
        windowsize_rolling_seas = seasonal_periods
...
def add_calendar_features(dataset: pd.DataFrame, event_lags: list, special_days: list, seasonal_periods: int = 0):
    """Function adding all calendar-based features"""
    DateCalendarFeatures.add_date_based_features(dataset=dataset, seasonal_periods=seasonal_periods)
    if 'public_holiday' in dataset.columns:
        DateCalendarFeatures.add_valentine_mothersday(dataset=dataset)
        DateCalendarFeatures.add_public_holiday_counters(dataset=dataset, event_lags=event_lags,
